...
fn render_systemd_unit(config: &AppConfig, app: &AppName) -> Result<String> {
    ...
    // Resource backstops. These are generous ceilings, not a tight leash: the
    // goal is that one runaway service can't take down the box and every other
    // service on it. `50%` is a fraction of physical RAM, so it scales with the
    // box and is effectively unreachable for a normal Discord bot.
    let memory_max = memory_max.clone().unwrap_or_else(|| "50%".into());
    let tasks_max = tasks_max.unwrap_or(4096);
    ...
    // Hardening defaults. Tuned for a network-egress service (e.g. a Discord
    // bot) that reads only its working dir + injected env file and talks to the
    // outside world. Every directive here is safe for that shape; anything that
    // would need inbound binding to a privileged port or local IPC beyond the
    // env file is an explicit opt-in the operator can add via a drop-in.
    ...
    // The service never needs the age store directly — secrets arrive via the
    // env file systemd injects. Make the store genuinely unreachable so a
    // compromised service can't read other apps' secrets; this turns the
    // per-app secret allowlist into a real boundary rather than just hygiene.
    // The leading `-` tolerates the path not existing yet.
    s.push_str("InaccessiblePaths=-%h/secrets\n");
    ...
    // Hide other users'/processes' entries under /proc, but deliberately NOT
    // ProcSubset=pid — that also hides /proc/meminfo, /proc/cpuinfo etc., which
    // some runtimes read to size thread pools.
    s.push_str("ProtectProc=invisible\n");
    // AF_NETLINK is required: glibc's getaddrinfo() opens a netlink socket to
    // enumerate interfaces, so dropping it breaks DNS resolution (and thus the
    // bot's connection to Discord).
    s.push_str("RestrictAddressFamilies=AF_UNIX AF_INET AF_INET6 AF_NETLINK\n");
    ...
    // Safe for native Rust binaries (no JIT). A service that genuinely needs
    // W+X memory must override this.
    s.push_str("MemoryDenyWriteExecute=yes\n");
    s.push_str("SystemCallFilter=@system-service\n");
    s.push_str("SystemCallErrorNumber=EPERM\n");
    s.push_str("SystemCallArchitectures=native\n");
    // Drop all capabilities — Shiku allocates high (unprivileged) ports, so a
    // service never needs CAP_NET_BIND_SERVICE or anything else.
    s.push_str("CapabilityBoundingSet=\n");
    s.push_str("AmbientCapabilities=\n");
    s.push_str("UMask=0077\n");
    s.push('\n');

    // Resource backstops (override per-app in shiku.toml [apps.<name>.systemd]).
    s.push_str("# Resource limits\n");
    s.push_str(&format!("MemoryMax={memory_max}\n"));
    s.push_str(&format!("TasksMax={tasks_max}\n"));
...
mod tests {
    ...
    fn unit_render_includes_essentials() {
        ...
        // Expanded hardening baseline.
        assert!(unit.contains("InaccessiblePaths=-%h/secrets"));
        ...
        // AF_NETLINK must be present or getaddrinfo()/DNS breaks.
        assert!(unit.contains("RestrictAddressFamilies=AF_UNIX AF_INET AF_INET6 AF_NETLINK"));
        // TasksMax backstop always rendered.
        assert!(unit.contains("TasksMax=4096"));
    }

    #[test]
    #[serial(home_env)]
    fn unit_render_applies_resource_backstops_when_unset() {
        std::env::set_var("HOME", "/home/user");
        let mut config = fake_config();
        config.systemd = SystemdSpec::default(); // no overrides
        let unit = render_systemd_unit(&config, &"test-app".into()).unwrap();
        // Generous default ceilings so one runaway service can't OOM the box,
        // without a tight leash that would surprise-kill a normal bot.
        assert!(unit.contains("MemoryMax=50%"));
        assert!(unit.contains("TasksMax=4096"));
        // CPUQuota stays opt-in — not emitted unless configured.
